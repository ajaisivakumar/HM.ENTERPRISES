...
router.get('/daily', ...requireRole('ADMIN'), async (req, res) => {
  try {
    ...
    sum.getCell('A1').value = `KB ENTERPRISES Daily Report — ${date}`;
    ...
    const inProgressCount = orders.filter(o => o.status === 'ACCEPTED').length;
    ...
    res.setHeader('Content-Disposition', `attachment; filename="KB-Enterprises-${date}.xlsx"`);
